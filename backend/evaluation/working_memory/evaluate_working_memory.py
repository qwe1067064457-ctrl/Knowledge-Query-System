...
from evaluation.working_memory.topic_config import build_topic_config, summarize_results  # noqa: E402
...
def validate_case(case: dict[str, Any]) -> None:
    required_fields = {
        ...
        "expected_focus_task_present",
        "expected_resolved_query_present",
        "expected_review_outcome_present",
    ...
    if not isinstance(case["working_memory_summary"], dict):
        raise ValueError("working_memory_summary must be an object")
    for key in (
        "expected_focus_task_present",
        "expected_resolved_query_present",
        "expected_review_outcome_present",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate session working memory continuity cases.")
