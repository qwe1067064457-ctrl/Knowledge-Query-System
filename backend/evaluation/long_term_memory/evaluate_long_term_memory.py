...
from evaluation.long_term_memory.topic_config import build_topic_config, summarize_results  # noqa: E402
...
def validate_case(case: dict[str, Any]) -> None:
    ...
    if not isinstance(case["memory_input_summary"], dict):
        raise ValueError("memory_input_summary must be an object")
    if not isinstance(case["persist_summary"], dict):
        raise ValueError("persist_summary must be an object")
    if not isinstance(case["expected_write"], bool):
        raise ValueError("expected_write must be a boolean")
    if case["expected_memory_type"] is not None and not isinstance(case["expected_memory_type"], str):
        raise ValueError("expected_memory_type must be a string or null")
    if case["expected_scope"] is not None and not isinstance(case["expected_scope"], str):
        raise ValueError("expected_scope must be a string or null")
    if not isinstance(case["anchor_before"], str):
        raise ValueError("anchor_before must be a string")
    if case["anchor_after"] is not None and not isinstance(case["anchor_after"], str):
        raise ValueError("anchor_after must be a string or null")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate long-term memory persistence cases.")
