...
from evaluation.long_term_memory.graders.finalize_layer.finalizer import LongTermMemoryFinalizer
from evaluation.long_term_memory.graders.model_layer.model_evaluator import LongTermMemoryModelEvaluator
from evaluation.long_term_memory.graders.rule_layer.rule_evaluator import LongTermMemoryRuleEvaluator
...
def build_topic_config(
    *,
    ...
    return TopicRunnerConfig(
        topic="long_term_memory",
        rule_evaluator=LongTermMemoryRuleEvaluator(),
        model_evaluator=LongTermMemoryModelEvaluator(
            use_llm=use_llm,
            label_overrides=label_overrides,
            llm_metadata_overrides=llm_metadata_overrides,
        ),
        finalizer=LongTermMemoryFinalizer(),
...
def _build_markdown_report(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Long-Term Memory Evaluation Report",
            "",
