...
6. 不要直接输出 deep binding result，不要重写 query，不要输出最终 resolved target。
7. `recommended_binding_mode` 只表达 binding strategy hint：
   - `skip`
   - `global_only`
   - `selective_per_unit`
8. 如果 `binding_scope_hint=partial`，优先输出 `recommended_binding_mode=selective_per_unit`。
9. 如果 `binding_scope_hint=none`，优先输出 `recommended_binding_mode=skip`。
10. 如果你认为存在 shared target，也应保持保守，除非证据足够强。
