...
def test_global_binding_frame_prefers_llm_structured_result_when_available() -> None:
          ...
          "segment_hints": [
            {
              "text": "把刚才那个结论展开一下",
              "needs_context": true,
              "segment_type": "follow_up_targeted",
              "context_need_type": "reference_recovery",
              "shared_target_candidate_ids": ["question_1"],
              "reason": "shared target",
              "confidence": "high",
              "rewrite_hint": "展开上一个结论并补依据",
              "inherited_context_span": "assistant:recent"
            }
          ],
          "notes": ["llm_frame"]
        }
    ...
    assert frame.segment_hints[0]["segment_type"] == "follow_up_targeted"
    assert frame.segment_hints[0]["context_need_type"] == "reference_recovery"
...
def test_global_binding_rule_frame_emits_richer_segment_hint_schema() -> None:
    worker = GlobalBindingWorker()

    frame = worker.build_frame(
        query="刚才那个结论具体指什么？另外总结一下",
        candidates=[{"object_id": "question_1", "content": "刚才那个结论"}],
        recent_messages=[{"role": "assistant", "content": "前面提到了一个结论。"}],
    )

    assert frame.segment_hints
    first_hint = frame.segment_hints[0]
    assert first_hint["segment_type"] == "follow_up_targeted"
    assert first_hint["context_need_type"] == "reference_recovery"
    assert first_hint["confidence"] in {"medium", "high"}
    assert "rewrite_hint" in first_hint
...
def test_execution_worker_uses_capability_executor_notes_for_working_memory_consumption() -> None:
    ...
    assert result.unit_results[0].result_payload["judgment"] == "pending_verification"
    assert result.unit_results[1].result_payload["confidence"] == "medium"
