...
class GroupServiceTests(unittest.TestCase):
    ...
    def test_create_group_initializes_storage_members_and_knowledge_layout(self) -> None:
        with temp_backend_dir() as backend_dir:
            ...
            self.assertTrue((backend_dir / "storage" / "groups" / "law" / "registries" / "history" / "source_registry.jsonl").exists())
            self.assertTrue((backend_dir / "storage" / "groups" / "law" / "registries" / "recovery" / "scan_checkpoints.sqlite").exists())
